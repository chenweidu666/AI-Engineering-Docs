...
### 1.14.1. 使用 ModelScope 下载 `Qwen2-0.5B-Instruct`
...
### 1.17.1. `ValueError: Invalid repository ID`
- **原因**：模型路径或 ID 错误。
- **解决**：检查模型路径或 ID 是否正确。
...
## 1.19. 📎 参考资料：国产GPU适配vLLM完整指南

## 1.20. 国产 GPU 适配 vLLM 完整指南
...
## 1.23. vLLM Platform Plugin 系统

### 1.23.1. 1 Plugin 系统架构
...
### 1.26.4. 4 Rotary Embedding 算子适配
...
### 1.26.5. 5 使用 torch.library 注册算子
...
### 1.28.1. 1 DeviceCommunicator 实现
