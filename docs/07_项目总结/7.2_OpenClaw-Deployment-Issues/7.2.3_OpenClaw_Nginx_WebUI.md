...
<div style="text-align: center; font-size: 2rem; font-weight: 700; margin-bottom: 0.5rem;"><strong>Nginx HTTPS：局域网 Web UI 访问</strong></div>

本文档从 [环境构建与 API 配置](./7.2.1_OpenClaw_Deploy_Guide.md) 中独立出来，详细介绍如何通过 Nginx 反向代理让局域网设备访问 OpenClaw Web UI。
...
- [1. 概述](#2-概述)
- [2. 生成自签名 SSL 证书](#3-生成自签名-ssl-证书)
- [3. 配置 Nginx](#4-配置-nginx)
- [4. 配置 OpenClaw 允许外部来源](#5-配置-openclaw-允许外部来源)
- [5. 效果展示](#6-效果展示)
...
> **关键点**：`/ws` location 必须将 token 拼接到 `proxy_pass` URL 中，否则 WebSocket 连接会因缺少认证而失败。详见 [踩坑记录](./7.2.2_OpenClaw_Pitfalls_and_Practices.md#坑-3nginx-websocket-要注入-token)。
