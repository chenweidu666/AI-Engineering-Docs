<p align="center"><strong>Dify 工作流平台</strong></p>



> **导航**：[← 5.2 MCP 协议](5.2_MCP协议.md) · [下一节：5.4 AppAgent →](5.4_AppAgent.md)

> **本章导读**：Dify 将 **Prompt、工作流、RAG、Agent、发布与观测** 放在同一套产品里，适合从验证到中小规模生产；与「纯代码框架」相比牺牲部分自由度，换取 **协作效率与运维一体化**。正文抓主线；**长案例与竞品对比**见附录。

> **关键词**：Dify · Workflow · Chatflow · RAG · Agent Node · Plugin

---

<h1 align="center">Dify 是什么</h1>

**Dify**（社区驱动、LangGenius 开源）是一套 **LLM 应用开发平台**：可视化编排应用逻辑、托管知识库与模型凭据、提供 API / WebApp / 嵌入发布，并内置日志与运营能力。定位介于 **低代码 SaaS** 与 **手写 LangChain** 之间，强调 **可上线的工程闭环**。
...
- **Workflow**：偏 **批处理/单轮任务**，输入经固定或有分支的节点图得到输出。  
- **Chatflow**：面向 **多轮对话**，可携带会话变量、历史与 Agent 多步推理。  
- 二者都可接 **条件分支、循环、变量与外部 HTTP**，适合把业务规则画在画布上由非纯研发角色参与。

## 知识库（RAG）

- 内置文档分段、清洗、Embedding、向量检索与 **重排序** 配置；支持多知识库与引用展示。  
- 与 [4.1 RAG](../04_RAG与知识库/4.1_RAG检索增强生成.md) 理论对应：Dify 把链路产品化，复杂自定义仍可能要 **代码节点** 或外挂服务。

## Agent 应用

- **Agent 节点** 上可挂多种工具（内置 + 自定义 HTTP/OpenAPI），策略含 **Function Calling / ReAct** 等（随版本迭代以官方文档为准）。  
- 适合 **「检索 + 多工具 + 多轮澄清」** 类助手；超长链路建议拆 **子工作流** 或引入外部编排。

---

<h1 align="center">工作流节点类型（常用）</h1>

| 类型 | 作用 | 提示 |
|------|------|------|
| **LLM** | 单步生成、分类、抽取 | 控制 max_tokens、温度与输出格式（JSON 模式） |
| **知识检索** | 向量/关键词检索知识库 | 与 Rerank、top_k、score 阈值联动调优 |
| **HTTP 请求** | 调用外部 REST | 统一错误处理、超时、敏感头脱敏 |
| **代码** | Python 等轻量逻辑 | 适合格式转换、签名校验，避免把业务全堆在提示词里 |
| **工具 / Agent** | 多步工具调用 | 限制工具集与最大迭代步数，防止成本失控 |
| **变量与赋值** | 会话级/单次运行状态 | 多轮场景显式设计变量，勿全靠模型「记住」 |

---

<h1 align="center">实战：BTC / 加密市场行情分析工作流（抽象）</h1>

1. **触发**：用户输入交易对 + 周期，或定时任务拉取列表。  
2. **检索节点**：从自建知识库取 **指标释义、风控规则、研报摘要**（非实时价格源）。  
3. **HTTP 节点**：调用交易所或行情 API 取 **K 线、资金费率、深度**（注意频率限制与合规）。  
4. **LLM 节点**：根据模板输出 **结构化结论**（趋势判断、风险点、免责声明）。  
5. **后置**：可选推送到飞书/邮件；**日志**记录每次引用到的 API 与知识库片段，便于复盘。  

**注意**：涉及 **投资建议** 的展示需合规审核；自动化交易应独立于 Dify，在专用风控系统中执行。

---

<h1 align="center">与 OpenClaw / 自研 Agent 对比</h1>

| 维度 | Dify | OpenClaw / 自研 Agent |
|------|------|------------------------|
| 交付形态 | 可视化 + 托管组件 | 代码优先，高度定制 |
| MCP | 视版本以 Plugin/工具形式衔接 | 原生多 MCP Server 组合常见 |
| 多 Agent 复杂图 | 画布 + 子流程，极复杂时可能吃力 | LangGraph 等代码编排更灵活 |
| 运维 | 自带用户、计费、日志面板 | 需自建可观测与权限 |

**选型**：**快速验证、业务参与迭代** 倾向 Dify；**7×24 自主任务、强状态机、深度 MCP** 倾向代码栈。二者也可 **Dify 做对外问答，自研服务做执行面**。

---

<h1 align="center">踩坑记录</h1>

- **向量库与备份**：自托管时 Postgres / Redis / 向量引擎要纳入 **备份与升级策略**。  
- **提示词泄露**：多租户场景注意 **知识库权限** 与 **日志脱敏**。  
- **Agent 死循环**：务必配置 **最大工具步数** 与 **单次对话 token 上限**。  
- **版本升级**：大版本迁移前在 **预发环境** 回归工作流 JSON。  

---

<h1 align="center">附录：Dify 平台技术实践（扩展阅读）</h1>

<h1 align="center">Dify 技术介绍</h1>
...
<h1 align="center">什么是 Dify？</h1>

| 属性 | 内容 |
|------|------|
| **全称** | Dify（源自 "Do It For You" 或 "Define + Modify"） |
| **提出者 / 维护** | LangGenius 社区与 Dify 团队（开源项目 **langgenius/dify**；与 DeepSeek 无隶属关系） |
...
<h1 align="center">为什么需要 Dify？（痛点解决）</h1>
...
<h1 align="center">Dify 常见使用场景</h1>
...
<h1 align="center">实战案例：旅游推荐 Agent 工作流设计</h1>
...
> **下一节**：[5.4 AppAgent →](5.4_AppAgent.md)